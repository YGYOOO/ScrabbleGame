...
public class GUI_challenger extends JFrame implements MouseMotionListener, MouseListener
{
	/**
	 * 
	 */
...
//	private RoundPanel user1 = new RoundPanel("pictures/glass3.png",5);
	...
	private JTextField score3 = new JTextField();
	private JTextField score4 = new JTextField();
	...
	private JTextField user3Name = new JTextField();
	private JTextField user3Score = new JTextField();
	private JTextField user4Name = new JTextField();
	private JTextField user4Score = new JTextField();
	...
	public GUI_challenger(String inputname1,String inputname2, String inputname3,String inputname4, int numberOfUser, String tileSrc1, String font) throws FileNotFoundException
	{
		n = numberOfUser;
		tileSrc = tileSrc1;
		...
		if(n==3)
		{
			u[2] = new User(inputname3);
			u[3] = new User("");
		}
		else if(n==4)
		{
			u[2] = new User(inputname3);
			u[3] = new User(inputname4);
		}
...
//		confirm.setUI(new BEButtonUI().setNormalColor(BEButtonUI.NormalColor.lightBlue));
		...
		if(n==2)
		{
			...
			user3Name.setVisible(false);
			score3.setVisible(false);
			user3Score.setVisible(false);
			name4.setVisible(false);
			user4Name.setVisible(false);
			score4.setVisible(false);
			user4Score.setVisible(false);
		}
		else if(n==3)
		{
			name4.setVisible(false);
	...
	public void showTiles()
	{
		for(int i=0;i<7;i++)
		{
			...
			letter2[i].setFont(f1);
			letter2[i].setOpaque(false);
			if(u[times%n].getTile(i).getLetter().equals(""))
			{
				letter2[i].setText("");
			}
			else
			{
				letter2[i].setText(u[times%n].getTile(i).getLetter()+" "+u[times%n].getTile(i).getScore());
			}			
			letter[i].setFont(f1);
...
//		System.out.println(set.size());
		for(int i=0;i<7;i++)
		{
			int n1 = r.nextInt(set.size());
			int count = 0;
			for(Tile l:set)
			{
				if(count==n1)
				{
					if(u[times%n].getTile(i)==null)
					{
						set.remove(l);
						u[times%n].setTile(l, i);
						break;
					}
					else
					{
...
//			JOptionPane.showMessageDialog(null,"Words can be placed only horizontally or vertically.");
		...
		for(int i=0;i<7;i++)
		{
			if(coordinate[i][0]!=1000&&u[times%n].getTile(i).getLetter().equals(""))
			{
				Object[] options = {"OK"};
...
//						for(int j=0;j<yd-yu+1;j++)
//						{
//							score = score + Tiles[x][yu+j].getScore();
//						}	
...
//		        //TODO exception
//		    }
//		GUI_challenger GUI_Normal1 = new GUI_challenger();
//	}
